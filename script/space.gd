...
onready var platform_generator := $PlatformGenerator as PlatformMapGenerator
onready var map_chunk_width := int(get_viewport().size.x * platform_generation_chunk_size)
onready var ui := $UI as CanvasLayer
onready var score := $UI/ControlScore/Score as Label
onready var music := $Audio/Music as AudioStreamPlayer
